  ...
  return new ReadableStream({
    ...
    async pull(controller) {
      const {
        bytesRead,
        buffer
      } = await fileHandle.read(new Uint8Array(CHUNK_SIZE$1), 0, CHUNK_SIZE$1, position);
      if (bytesRead === 0) {
...
async function createBufferFile() {
  const bufferDir = path.join(tmpdir(), "sanity-migrate", "".concat(Date.now(), "-").concat(Math.random().toString(36).slice(2)));
  await mkdir(bufferDir, {
    recursive: true
  });
