...
const validateDocumentsCommand$1 = {
  ...
  action: async (args, context) => {
    const mod = await Promise.resolve().then(function () {
      return require('./validateAction-TQJ_lYJj.js');
    });
    return mod.default(args, context);
  }
};
...
const listGraphQLAPIsCommand = {
  ...
  action: async (args, context) => {
    const mod = await Promise.resolve().then(function () {
      return require('./listApisAction-Ppk8Ff15.js');
    });
    return mod.default(args, context);
  },
  helpText: helpText$7
...
const deployGraphQLAPICommand = {
  ...
  action: async (args, context) => {
    const mod = await Promise.resolve().then(function () {
      return require('./deployApiAction-Plxotpd8.js');
    });
    return mod.default(args, context);
  },
  helpText: helpText$6
...
const createMigrationCommand = {
  ...
  action: async (args, context) => {
    ...
    const destDir = path__default$1.default.join(MIGRATIONS_DIRECTORY, sluggedName);
    ...
    const definitionFile = path__default$1.default.join(destDir, "index.ts");
    await promises.writeFile(path__default$1.default.join(workDir, definitionFile), renderedTemplate);
    output.print();
...
function resolveMigrationScript(workDir, migrationName) {
  return [migrationName, path__default$1.default.join(migrationName, "index")].flatMap(location => MIGRATION_SCRIPT_EXTENSIONS.map(ext => {
    const relativePath = path__default$1.default.join(MIGRATIONS_DIRECTORY, "".concat(location, ".").concat(ext));
    const absolutePath = path__default$1.default.resolve(workDir, relativePath);
    let mod;
    try {
...
async function resolveMigrations(workDir) {
  ...
  const migrationsDir = path__default$1.default.join(workDir, MIGRATIONS_DIRECTORY);
  const migrationEntries = await promises.readdir(migrationsDir, {
    withFileTypes: true
  });
  const migrations = [];
...
function removeMigrationScriptExtension(fileName) {
  return MIGRATION_SCRIPT_EXTENSIONS.reduce((name, ext) => name.endsWith(".".concat(ext)) ? path__default$1.default.basename(name, ".".concat(ext)) : name, fileName);
}
...
//# sourceMappingURL=_internal-YPXVt6i_.js.map
