...
import { MAX_MUTATION_CONCURRENCY, run, dryRun, DEFAULT_MUTATION_CONCURRENCY } from '@sanity/migrate';
...
const copyDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    const flags = await parseCliFlags$4(args);
    const client = apiClient();
    if (flags.list) {
...
const validateDocumentsCommand$1 = {
  ...
  helpText: helpText$g,
  action: async (args, context) => {
    const mod = await import('./validateAction-HJyYe8Hk.js');
    return mod.default(args, context);
  }
};
...
const listGraphQLAPIsCommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./listApisAction-ojD1gX_c.js');
    return mod.default(args, context);
  },
  helpText: helpText$7
};
...
const deployGraphQLAPICommand = {
  ...
  action: async (args, context) => {
    const mod = await import('./deployApiAction--1_FKA9a.js');
    return mod.default(args, context);
  },
  helpText: helpText$6
};
...
const createMigrationCommand = {
  ...
  action: async (args, context) => {
    ...
    const destDir = path$1.join(MIGRATIONS_DIRECTORY, sluggedName);
    ...
    const definitionFile = path$1.join(destDir, "index.ts");
    await writeFile(path$1.join(workDir, definitionFile), renderedTemplate);
    output.print();
...
function resolveMigrationScript(workDir, migrationName) {
  return [migrationName, path$1.join(migrationName, "index")].flatMap(location => MIGRATION_SCRIPT_EXTENSIONS.map(ext => {
    const relativePath = path$1.join(MIGRATIONS_DIRECTORY, "".concat(location, ".").concat(ext));
    const absolutePath = path$1.resolve(workDir, relativePath);
    let mod;
    try {
...
async function resolveMigrations(workDir) {
  ...
  const migrationsDir = path$1.join(workDir, MIGRATIONS_DIRECTORY);
...
function removeMigrationScriptExtension(fileName) {
  return MIGRATION_SCRIPT_EXTENSIONS.reduce((name, ext) => name.endsWith(".".concat(ext)) ? path$1.basename(name, ".".concat(ext)) : name, fileName);
}
  ...
  action: async (args, context) => {
    ...
    if (concurrency !== void 0) {
      if (concurrency > MAX_MUTATION_CONCURRENCY) {
        throw new Error("Concurrency exceeds the maximum allowed value of ".concat(MAX_MUTATION_CONCURRENCY));
      }
      if (concurrency === 0) {
...
//# sourceMappingURL=_internal-reJWSY4V.js.map
