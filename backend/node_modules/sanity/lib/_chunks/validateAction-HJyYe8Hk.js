...
import { convertToTree, maxKeyLength, formatTree } from './_internal-reJWSY4V.js';
...
function formatDocumentValidation(_ref3) {
  ...
  const header = "".concat(levelHeaders[level], " ").concat(documentTypeHeader, " ").concat(intentUrl ? link(documentId, intentUrl) : chalk.underline(documentId));
...
const json = async _ref7 => {
  ...
  const formatted = results.filter(_ref8 => {
    let {
      markers
    } = _ref8;
...
async function validateAction(args, _ref10) {
  ...
  if (!unattendedMode) {
    output.print("".concat(chalk.yellow("".concat(logSymbols.warning, " Warning:")), " This command ").concat(flags.file ? "reads all documents from your input file" : "downloads all documents from your dataset", " and processes them through your local schema within a ") + "simulated browser environment.\n");
...
//# sourceMappingURL=validateAction-HJyYe8Hk.js.map
